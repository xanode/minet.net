...
A MAC address is typically represented in hexadecimal form like <Code code='5A-FF-56-A2-AF-15' inline/> or <Code code='5A-FF-56-A2-AF-15' inline/>. There are various methods to find it, depending on the operating system you are using. We have listed the most common ones below.

### Windows

- Press the following key combination <KBD keys={["Windows", "R"]} /> to open the *Run* window.
- Type <Code code='cmd' inline/> and then press <KBD keys={["Enter"]} /> to open the command prompt.
- Type <Code code='getmac -v' inline/> and then press <KBD keys={["Enter"]} /> to obtain a list of the device's MAC addresses.
...
- Open a terminal.
- Type <Code code='ip link show' inline/> followed by <KBD keys={["Enter"]} /> for a summary of the device's interfaces.
...
- In the search bar, type <Code code='terminal' inline/> and then press <KBD keys={["Enter"]} />.
- Then type <Code code='networksetup -listallhardwareports' inline/> followed by <KBD keys={["Enter"]} />.
