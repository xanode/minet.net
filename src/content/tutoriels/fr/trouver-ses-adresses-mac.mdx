...
Une adresse MAC est généralement représentée sous forme hexadécimale du type <Code code='5A-FF-56-A2-AF-15' inline/> ou <Code code='5A:FF:56:A2:AF:15' inline/>. Il existe différents moyens de la trouver, selon le système d'exploitation utilisé. Nous avons listé ci-après les plus communs.

 
### Windows

- Faites la combinaison de touches suivante <KBD keys={["Windows", "R"]} /> pour ouvrir la fenêtre *Exécuter*.
- Tapez <Code code='cmd' inline/> puis <KBD keys={["Entrer"]} /> pour ouvrir l'invité de commande.
- Tapez <Code code='getmac -v' inline/>, puis <KBD keys={["Entrer"]} /> pour obtenir une liste des MACs de l'appareil.
...
- Ouvrez un terminal.
- Tapez <Code code='ip link show' inline/> suivi de <KBD keys={["Entrer"]} /> pour un résumé des interfaces de l'appareil
...
- Dans la barre de recherche, tapez <Code code='terminal' inline/> puis <KBD keys={["Entrer"]} />
- Tapez ensuite <Code code='networksetup -listallhardwareports' inline/> suivi de <KBD keys={["Entrer"]} />
